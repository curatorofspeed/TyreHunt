...
# Car and Bike of the Day; the missed-car queue removed; capture lookup fix — 2026-09-26
**Trigger:** Drew: "when a user is hunting 'both', why not show both car and bike of the day?" and "we can be without the missed car queue… it kinda devalues the car of the day. You get one day to find it."

## What changed
- **Both lanes for "both" hunters.** The 90/10 lane roll is gone from the app. `cotdPick(lane)` is the day's pick for a lane (server row first, rotation as fallback); `carOfTheDay()` is the primary lane's (car for "both"), `bikeOfTheDay()` the bike for "both" hunters only, `todaysPicks()` both. Hunts tab: a Car of the day card and a Bike of the day card. Hunt chip: the first unstamped pick. Stamp: either pick, same day. Feed's Car of the Day board: both cx_keys, head shows the bike under the car with its own SHARE TODAY'S BIKE. Poster and labels say BIKE OF THE DAY for a bike (this also fixes bike-only hunters, whose card said CAR OF THE DAY). Car-only and bike-only hunters see one pick as before.
- **8am push.** `cotd_push_due` (migration `cotd_push_both_lanes`) returns lane 'both' plus a `bike` column for both-hunters; `cotd-push` titles bike-lane pushes "Bike of the Day" and adds "Bike of the Day: X. Spot either before midnight." for both-hunters.
- **The queue is gone.** `loadCotdPast`, `cotdEntryFor`, `cotdQueue`, `cotdWhen`, `cotdAgoLabel`, the "Still on your list" fold and the late-stamp branch; `state.cotdPast` deleted on load. Stamps already earned from the queue stay. `car_of_the_day_range` stays in the database, unused.
- **Capture lookup fix (pre-existing since July).** `processResult` looked up the Registry with `{make, model}` only; `findEntry` takes the lane from tags and prefers the Paddock for race-tagged captures, so every bike was looked up among the cars (never a numbered bike, never a bike-day stamp, never on the bike board) and every race car missed the Paddock. The lookup now carries the judge's tags. A one-time boot pass (`state.rekeyedTags`) re-files local spots whose key is the old tag-less result and differs from the tagged one. Server: capture #150 (Mercedes-AMG GT3) re-filed to n107; #147 (Harley Softail) stays loose, the bike Registry has no Softail.

## Verified
Preview: car, moto and both hunters get the right picks, chip, cards and share buttons, no queue fold; a bike capture files as m20 on the phone and the row, and stamps gold BIKE OF THE DAY; a race car files n107; the car still stamps; the chip moves on once both are stamped; the rekey pass moves only wrongly filed spots. SQL: today's push for car / moto / both hunters names Alfa Romeo / BSA Gold Star / Alfa Romeo + BSA Gold Star. Push function dry run through pg_net: 200.

**Review before shipping** (3 lenses, 16 findings, the 7 most severe checked adversarially; 6 confirmed, 1 split): a pick stamped on an earlier day showed "Stamped today" when it came round again and could not be earned → stamps now count for the day they were earned (`cotdStampedToday`), and re-stamping moves the date; the re-file pass fixed phones only → it marks moved spots and `backfillSync` sends the corrected key to the server, and it also updates the Registry number; the beta page still promised the missed-car list → copy updated; a bike opened in the Registry as "FIELD ENTRY" → `entryInfo` resolves m-keys; the both-hunter push now leads with the bike so a collapsed notification shows it; "Stamped today" and "STAMPED" gained translations. Not done (low): bike-only hunters still see "car" in the 8am offer and the Feed tab name; the combined board ranks car and bike shots together.
